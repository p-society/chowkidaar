...
def mark_db(
    studentId, questions, name, leetcode_submissions, codeforces_submissions, day
):
    conn = connect_db()
    if not conn:
        return
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            'SELECT * FROM "student_list_2024" WHERE "stu_id" = %s;', (studentId,)
        )
...
def delete_db(studentId, day):
    ...
    with conn.cursor() as cur:
        ...
        conn.commit()
        print(
            f"✅ Deleted '{day}' from {count_removed} field(s) for student '{studentId}'."
        )
