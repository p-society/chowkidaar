...
def is_unique_in_time_bracket(discord_user_id, msg_sending_time, conn):
    ...
    if msg_sending_time_ist.time() >= time(22, 0):
        bracket_start = msg_sending_time_ist.replace(hour=22, minute=0, second=0, microsecond=0)
        bracket_end = (bracket_start + timedelta(hours=14)).replace(hour=12, minute=0, second=0, microsecond=0)
    else:
        bracket_start = (msg_sending_time_ist.replace(hour=22, minute=0, second=0, microsecond=0) - timedelta(days=1))
        bracket_end = msg_sending_time_ist.replace(hour=12, minute=0, second=0, microsecond=0)
