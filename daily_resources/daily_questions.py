...
def load_questions():
    try:
        ...
        logger.error(
            f"Error loading questions.json: {e}",
            extra={"tags": {"event": "load_questions"}},
        )
        return {}
...
class DailyQuestionScheduler:
            ...
            if 0 < day <= 25:
                ...
                if day_str in self.questions:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Sent day {day} dev questions to channel",
                    ...
                    logger.error(
                        f"No questions found for day {day}",
                        extra={"tags": {"event": "daily_question"}},
                    )
        except Exception as e:
            logger.error(
                f"Error in daily question task: {e}",
                extra={"tags": {"event": "daily_question"}},
            )
    ...
    async def before_daily_task(self):
        ...
        logger.info(
            f"Daily question scheduler initialized. Starting date: {self.start_date}",
