...
def extract_user_lc_cf_id(msg: str) -> Dict[str, str | None]:
    """
    ...
    lc_patterns = [r"(?i)lc:\s*(\w+)", r"(?i)lc\s*-\s*(\w+)", r"(?i)lc-(\w+)"]

    cf_patterns = [r"(?i)cf:\s*(\w+)", r"(?i)cf\s*-\s*(\w+)", r"(?i)cf-(\w+)"]
...
def extract_day_number(msg: str) -> int | None:
    """
...
def is_registration_message(msg: str) -> bool:
    ...
    A registration message must start with 'Register User' (on the first line),
    and contain both LC and CF handles.
